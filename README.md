...
React is a component library that allows us to make code that is predictable, reusable, and easier to debug. We can build components that can manage their own state and then pass that state down to child components as props. It solves a performance problem by using the Virtual DOM instead of directly acting on the DOM itself.

- [x] What does it mean to _think_ in react?

  Looking at a project and breaking it down into components that can be reuasable. Each comonent should have one responsibility.

- [x] Describe state.

  State is the properties we give the parent component that can then be used by our methods and can be passed down to child components as props.

- [x] Describe props.

  Props are the properties that are passed down to a child component from a parent that can then be used by the child.
